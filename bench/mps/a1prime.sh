...
echo "arm,round,geometry,seconds,max_rss_bytes,peak_footprint_bytes" > "$SD/mps-g1-a1prime.csv"
for r in 0 1 2 3; do   # r=0 is the discarded warm-up
  log="$SD/run-g1-A1p-$r.log"
  ( cd "$SW" && LTX_TIER=max128 LTX_QUANT=int8 LTX_T2V_PROMPT="$PROMPT" \
  ...
  [ "$r" != "0" ] && echo "A1p,$r,704x512x33,$sec,$rss,$fp" >> "$SD/mps-g1-a1prime.csv"
  echo "[a1p] round$r ${sec}s"
done
