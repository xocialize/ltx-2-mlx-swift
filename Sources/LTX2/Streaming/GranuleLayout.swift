// HV2 weight streaming, part 1: the granule layout — LTX-2.3 adaptation of
// wan-core-mlx-swift's GranuleLayout (commit b45b879; NEUROSTREAM-ACTIONS HV2,
// STREAMING-PLAN.md). Deliberately a sibling implementation, NOT a wan-core
// dependency — extraction into a shared substrate package is sequenced for
// after this second consumer's receipts pass (plan §Generalization).
//
...
// `LTXBlockStreamer` preads into a live slot between group boundaries. This
// file owns:
...
// (bf16 blocks are 86 tensors / ~774 MB; the q8/q4 checkpoints' packed
// weight+scales+biases blocks are 154 tensors / ~411 / ~218 MB). Both source
// reads and granule writes run F_NOCACHE: a 38 GB checkpoint must not flush the
// UBC on layout, and honest-cold streaming receipts depend on granules never
// entering the page cache (the prototype's cold-number recipe).
...
/// Reads only the header — never the tensor data (the 38 GB DiT parses in ms).
...
/// The granule directory manifest — everything the `LTXBlockStreamer` needs to
        ...
        guard sourceSize == size else {
            throw GranuleLayoutError.badManifest(
                "granule source size \(sourceSize) != checkpoint size \(size) — "
                    + "stale granule tree? re-run ltx-granule-layout")
        }
    }
}
...
public enum GranuleLayout {
    ...
    /// The LTX-2.3 checkpoints' on-disk per-block prefix (`DiT.init` strips the
    /// outer `transformer.`; granules are laid out from the on-disk keys).
    public static let ltxBlockPrefix = "transformer.transformer_blocks."

    /// Canonical within-block tensor order: BasicAVTransformerBlock forward order
    /// (DiT.swift `block(_:)`) so a granule read visits tensors roughly as compute
    /// will — the AdaLN tables first (unpacked at block start), then the eight
    /// attention/FF stages in execution order. Suffix-prefix ranked; unknown
    /// suffixes sort after the known ones; lexicographic within a rank. Order
    /// affects locality only — correctness never depends on it (a slot is fully
    /// refilled before compute touches any of it).
    static let forwardOrderPrefixes: [String] = [
        "scale_shift_table_a2v_ca_video",  // before the bare-prefix tables below
