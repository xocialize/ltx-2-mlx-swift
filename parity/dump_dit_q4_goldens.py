#!/usr/bin/env python
"""q4 DiT parity fixture: int4-quantized transformer forward (bf16 activations).

Loads the real q4 distilled transformer (transformer-blocks Linears int4, group_size 64;
everything else bf16) into the oracle LTXModel via apply_quantization (bits auto-detected from
the scales shape), runs ONE forward on the SAME inputs as the dit_full fixture, and dumps the
q4 velocities. The Swift q4 DiT loads the same q4 weights and compares.

Run in the oracle uv env:
    cd ~/Development/mlxengine-video/LTX_DEV/ltx-2-mlx && \
        uv run python ../ltx-2-mlx-swift/parity/dump_dit_q4_goldens.py
"""
...
def main() -> None:
    ...
    cfg = LTXModelConfig.from_checkpoint_dir(Q4_DIR)
    print("config: layers=%d video_dim=%d audio_dim=%d" % (cfg.num_layers, cfg.video_dim, cfg.audio_dim))
    model = LTXModel(cfg)
    w = load_split_safetensors(Q4_DIR / "transformer-distilled.safetensors", prefix="transformer.")
    apply_quantization(model, w, group_size=64)  # bits auto-detected (4) from scales shape
